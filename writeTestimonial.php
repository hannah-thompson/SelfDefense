...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if(empty($_POST['title'])){
        $title_error = "PHP: Please provide a title";
    }
    else{
        $title_error = NULL;
        $title = $_POST['title'];
        ...
        if(empty($_POST['testimonial'])){
            $test_error = "PHP: Please provide your testimonial";
        }
        else{
            $test_error = NULL;
...
function addTestimonial($user, $title, $author, $test){
    ...
    $query = "INSERT INTO testimonials (username, title, author, testimonial) VALUES (:username, :title, :author, :testimonial)";
